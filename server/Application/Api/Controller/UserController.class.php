...
class UserController extends BaseController {
    ...
    public function login(){
        ...
        if ($ret) {
          ...
          $resultData = array();
          if ($custom_logo_open !== false) {//如果强等于false，那就是尚未有数据。
              if($custom_logo_open==1){
                  $custom_logo_form = D("Options")->get("custom_logo_form" ) ;
                  $resultData['custom_logo_form'] = $custom_logo_form;
              }
              $outside_btn_open = D("Options")->get("outside_btn_open" ) ;
              if($outside_btn_open==1){
                  $outside_btn_form = D("Options")->get("outside_btn_form" ) ;
                  $resultData['outside_btn_form'] = $outside_btn_form;
              }
          }
